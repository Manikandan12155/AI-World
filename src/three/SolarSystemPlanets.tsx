...
export const SolarSystemPlanets: React.FC<SolarSystemProps> = ({
  ...
  const textures = useMemo(() => {
    ...
    jupCanvas.width = 1024;
    jupCanvas.height = 512;
    const jctx = jupCanvas.getContext('2d')!;
    const bandColors = [
      '#78350f', '#d97706', '#fef3c7', '#b45309', '#fde68a',
      '#92400e', '#fef08a', '#b45309', '#fed7aa', '#78350f'
    ];
    const bandH = 512 / bandColors.length;
    bandColors.forEach((color, idx) => {
      jctx.fillStyle = color;
      jctx.fillRect(0, idx * bandH, 1024, bandH + 2);
    });
    for (let y = 0; y < 512; y += 16) {
      jctx.fillStyle = y % 32 === 0 ? 'rgba(255,255,255,0.18)' : 'rgba(120,53,15,0.2)';
      jctx.beginPath();
      jctx.moveTo(0, y);
      for (let x = 0; x <= 1024; x += 32) {
        jctx.lineTo(x, y + Math.sin(x * 0.04 + y) * 5);
      }
      jctx.lineTo(1024, y + 16);
      jctx.lineTo(0, y + 16);
      jctx.fill();
    }
    jctx.fillStyle = '#dc2626';
    jctx.beginPath();
    jctx.ellipse(640, 340, 48, 28, -0.15, 0, Math.PI * 2);
    jctx.fill();
    jctx.fillStyle = '#ef4444';
    jctx.beginPath();
    jctx.ellipse(640, 340, 32, 18, -0.15, 0, Math.PI * 2);
    jctx.fill();

    // 3. Saturn Texture & Rings
    ...
    sGrad.addColorStop(0, '#ca8a04');
    sGrad.addColorStop(0.3, '#fef08a');
    sGrad.addColorStop(0.5, '#eab308');
    sGrad.addColorStop(0.7, '#fde047');
    sGrad.addColorStop(1, '#a16207');
    ...
    rGrad.addColorStop(0.12, 'rgba(234, 179, 8, 0.4)');
    rGrad.addColorStop(0.42, 'rgba(254, 240, 138, 0.85)');
    rGrad.addColorStop(0.48, 'rgba(234, 179, 8, 0.9)');
    rGrad.addColorStop(0.52, 'rgba(0,0,0,0)');
    rGrad.addColorStop(0.58, 'rgba(202, 138, 4, 0.7)');
    rGrad.addColorStop(0.92, 'rgba(254, 240, 138, 0.5)');
    ...
    nepCanvas.width = 256;
    nepCanvas.height = 128;
    const nctx = nepCanvas.getContext('2d')!;
    const nGrad = nctx.createLinearGradient(0, 0, 0, 128);
    nGrad.addColorStop(0, '#0284c7');
    nGrad.addColorStop(0.5, '#38bdf8');
    nGrad.addColorStop(1, '#0369a1');
    nctx.fillStyle = nGrad;
    nctx.fillRect(0, 0, 256, 128);
    nctx.fillStyle = 'rgba(255,255,255,0.35)';
    nctx.fillRect(40, 50, 120, 6);
    nctx.fillRect(140, 75, 90, 4);

    // 6. Mercury Texture
    const merCanvas = document.createElement('canvas');
    merCanvas.width = 256;
    merCanvas.height = 128;
    const merctx = merCanvas.getContext('2d')!;
    merctx.fillStyle = '#64748b';
    merctx.fillRect(0, 0, 256, 128);
    merctx.fillStyle = 'rgba(30, 41, 59, 0.4)';
    for (let i = 0; i < 30; i++) {
      merctx.beginPath();
      merctx.arc((i * 47) % 256, (i * 29) % 128, 6 + (i % 4) * 3, 0, Math.PI * 2);
      merctx.fill();
    }

    return {
      jupiter: new THREE.CanvasTexture(jupCanvas),
      saturn: new THREE.CanvasTexture(satCanvas),
      saturnRing: new THREE.CanvasTexture(ringCanvas),
      venus: new THREE.CanvasTexture(venCanvas),
      neptune: new THREE.CanvasTexture(nepCanvas),
      mercury: new THREE.CanvasTexture(merCanvas),
    };
  }, []);
